...
public class TelaCentralNotificacoesController {
    ...
    public void initialize() {
        ...
        List<Notificacao> historicos = NotificacaoService.getInstance()
                .getNotificacoes(usuarioLogado.getId(), Notificacao.Tipo.HISTORICO);
        listHistorico.getItems().setAll(historicos.stream().map(Notificacao::toString).toList());

        List<Notificacao> alertas = NotificacaoService.getInstance()
                .getNotificacoes(usuarioLogado.getId(), Notificacao.Tipo.ALERTA);
